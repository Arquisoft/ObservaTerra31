...
public class Application extends Controller {
	...
	public static Result authenticate() {
		...
		if (loginForm.hasErrors())
			...
			Volcado.getInstance().volcar("[Login]", email, pass, new Date().toString());
	...
	public static Result registerAut() {
		...
		Form<User> loginForm = Form.form(User.class).bindFromRequest();
		 if (loginForm.hasErrors())
			...
			User.create(user);
			String pass = Hashin.md5(password);
			Volcado.getInstance().volcar("[Register]", name, surname, email, pass, new Date().toString());
